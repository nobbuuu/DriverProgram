...
public class DrivingRouteOverlay extends RouteOverlay{
    ...
    private int getcolor(String status) {
    	...
    	if (status.equals("畅通")) {
            return Color.parseColor("#00b769");
//    		return Color.GREEN;
		} else if (status.equals("缓行")) {
            return Color.parseColor("#f4c300");
//			 return Color.YELLOW;
    ...
    private List<Marker> mStationMarkerList = new ArrayList<>();
    ...
    protected void addStartAndEndMarker() {
        Log.d("addStartAndEndMarker", "addStartAndEndMarker");
        if(resIdStart != -1 && resIdEnd != -1){
            Log.d("addStartAndEndMarker", "bpStart != null && bpEnd != null");
            //起点,覆盖物可能是车，也可能是上车点图标
            startMarker = mAMap.addMarker(new MarkerOptions()
                    .position(startPoint)
                    .icon(BitmapDescriptorFactory.fromView(getBitmapView(mContext, resIdStart, startAddress))));
//                    .snippet("上车点描述"));
            //设置车辆作为起点Marker的特殊处理
            if(isCarStart){
                startMarker.setAnchor(0.5f, 0.5f);
                if(orientation >= 0){  //方向存在，则设置方向
                   // startMarker.setRotateAngle(orientation);
            ...
            mStationMarkerList.add(startMarker);

            //终点
            endMarker = mAMap.addMarker((new MarkerOptions()).position(endPoint)
                    .icon(BitmapDescriptorFactory.fromView(getBitmapView(mContext, resIdEnd, endAddress))));
...
//                    .snippet("上车点描述"));
    ...
    private BitmapDescriptor bpStart;
    private BitmapDescriptor bpEnd;
    private int resIdStart = -1;
    private int resIdEnd = -1;
    private String startAddress;
    private String endAddress;
    private float orientation = -1;  //车辆方向,范围[0,360],为-1不显示方向
    private boolean isCarStart;  //是否应该显示车的Marker
    ...
    public void setStartAndEndIcon(int start, int end) {
        bpStart = BitmapDescriptorFactory.fromResource(start);
        bpEnd = BitmapDescriptorFactory.fromResource(end);
        resIdStart = start;
        resIdEnd = end;
        orientation = -1;
        isCarStart = false;
     ...
     * @param ori   方向
     */
    public void setStartAndEndIcon(int start, int end,float ori) {
        bpStart = BitmapDescriptorFactory.fromResource(start);
        bpEnd = BitmapDescriptorFactory.fromResource(end);
        resIdStart = start;
        resIdEnd = end;
    ...
    public void setStartAndEndIcon(BitmapDescriptor start, BitmapDescriptor end) {
        bpStart = start;
        bpEnd = end;
    }
    ...
    public static View getBitmapView(Context context, int resId, String title)
    {
        ...
        ImageView iv_icon = (ImageView) view.findViewById(R.id.iv_marker);
