 ...
 */
public class CargoRestTimeView2 extends LinearLayout {
    ...
    public CargoRestTimeView2(Context context) {
        super(context);
        initView(context);
    }

    public CargoRestTimeView2(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }

    public CargoRestTimeView2(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView(Context context){
        mContext = context;
        View mRootView = LayoutInflater.from(mContext).inflate(R.layout.view_cargo_rest_time2, this);
    ...
    private void updateRestTimeView(){
        mTvRestTime.setText(BusinessUtils.formatEstimateTimeText(restTimeMillis/1000) + "内需将货物送达");
        if(restTimeMillis >  0) {
    ...
    private void updateRestTimeView(boolean restAndOverTime){
        ...
        if(restTimeMillis >= 0){
//            mTvRestTime.setText(BusinessUtils.formatEstimateTimeText(restTimeMillis/1000) + "内需将货物送达");
            mTvRestTime.setText(BusinessUtils.formatEstimateTimeText(restTimeMillis/1000, BusinessUtils.PrecisionMode.MINUTE) + "内需将货物送达");
